...
join employee on employee.employee_id = sales.employee_id 
join patient on patient.patient_id = sales.patient_id";
        $sql_query = mysqli_query($connect, $sql_fetch);
            ...
            ?>
                ...
                <tr>
                    ...
                    <td><?php echo $rows['quantity']; ?></td>
                                        <td>
                         <?php
                        $sql = "SELECT  stock.unit_price * sales.quantity as total_amount FROM stock,sales order by sales_id desc limit 1";
                 ...
                 <td>
                        ...
                        $sql = "SELECT (stock.unit_price * sales.quantity) - paid_amount AS remaining_amount FROM sales,stock ORDER BY sales_id DESC LIMIT 1";
                        $result = mysqli_query($connect, $sql);
                        $resultCheck = mysqli_num_rows($result);
                        if ($resultCheck > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
