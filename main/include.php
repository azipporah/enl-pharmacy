...
if(isset($_POST['Login'])){
	...
	$sql_fetch="SELECT * FROM user WHERE user_email='$user' AND user_password='$pwd'";
	$sql_query=mysqli_query($connect, $sql_fetch);
	$count=mysqli_num_rows($sql_query);
	 if ($count >0) {
	  // echo "FOUND";
...
if (isset($_POST['save-stock'])) {
	...
	$unit_price = mysqli_real_escape_string($connect, $unit_price);
	...
	$insert_stock = "INSERT INTO `stock`(`product_id`, `employee_id`, `stock_expiryDate`, `stock_quantity`, `unit_price`, `stock_description`, `productCategory_id`, `productUnit_id`) 
					VALUES ('$product_name','$employee_name','$stock_expiryDate','$stock_quantity','$unit_price','$stock_description','$product_category','$product_unit')";
...
// submitting data to the  sales database
 if (isset($_POST['save-sales'])) {
	...
	$insert_sales = "INSERT INTO `sales`(`employee_id`, `patient_id`, `product_id`, `productCategory_id`, `productUnit_id`, `stock_id`, `quantity`, `total_amount`, `paid_amount`, `remaining_amount`, `dateOfSale`) 
...
if(isset($_POST['save-appointment'])){
	...
	if ($execute_appointmentInsert === TRUE) {
...
// submitting data to the appointment database
if(isset($_POST['save-disease'])){
	$disease_name=$_POST['disease'];
	// $patient_id=mysqli_real_escape_string($conn, $patient_id);
	$affected_gender=$_POST['gender_affected'];
	// $dateOfAppointment=mysqli_real_escape_string($conn, $dateOfAppointment);
	
	$insert_disease = "INSERT INTO `disease`(`disease_name`, `affected_gender`) VALUES ('$disease_name','$affected_gender')";
	//executing the insert query
	// $execute_insertDisease = mysqli_query($connect, $insert_disease);

	if (mysqli_query($connect, $insert_disease)) {
		$New = "Disease added succesfully";
	} else {
		$Error = "Error" . $insert_disease . "</br>" . mysqli_error($connect);
	}
